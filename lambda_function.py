...
def lambda_handler(event, context):
    try:
        ...
        # Run the async scoring function with skill-based scoring enabled
        score = asyncio.run(scorer.calculate_total_score(submission, evaluator, use_skill_based_scoring=True))
...
def notify_recruitment_system(instance_id: str, skill_scores: dict, score_record: dict):
    ...
    try:
        ...
        technical_accuracy = skill_scores.get('technical_expertise', {}).get('score', 0.0)
        problem_solving = skill_scores.get('problem_solving', {}).get('score', 0.0)
        communication = skill_scores.get('communication', {}).get('score', 0.0)
